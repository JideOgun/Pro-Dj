...
export default function MixUpload({
    ...
    try {
      ...
      if (albumArt) {
        try {
          ...
          if (albumArtResponse.ok) {
            ...
            const { presignedUrl: albumArtPresignedUrl, albumArtS3Key: key } =
              albumArtData.data;
      ...
      if (completeResponse.ok) {
        ...
        if (response.status === 409) {
          ...
          // Show subscription upgrade prompt instead of error
  ...
  return (
    <>
      ...
      {/* Subscription Upgrade Prompt - Outside AnimatePresence */}
